...
export async function getCategoryBalances(organizationId: string, period: Period = {}) {
  const entries = await prisma.financialEntry.findMany({
    ...
    select: { lines: { select: { categoryId: true, amount: true, direction: true, category: { select: { id: true, name: true, classification: true } } } } },
  });
  ...
  return [...balances.values()].map((balance) => ({ ...balance, balance: fromCents(balance.cents) })).map(({ cents, ...balance }) => balance);
}
...
export async function getFinancialSummary(organizationId: string, period: Period = {}) {
  ...
  for (const category of categories) {
    ...
    if (category.classification === "EXPENSE") expense += cents;
    if (category.classification === "FUND") funds += cents;
  }

  return {
    income: fromCents(income),
    expenses: fromCents(expense),
    funds: fromCents(funds),
    netOperatingResult: fromCents(income - expense),
    categories,
  };
}
