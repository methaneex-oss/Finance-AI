...
export async function POST(request: NextRequest) {
  ...
  if (!name || !VALID_CLASSIFICATIONS.includes(classification as Classification)) return NextResponse.json({ error: "name and a valid classification are required" }, { status: 400 });
  if (startsAt === "invalid" || endsAt === "invalid") return NextResponse.json({ error: "startsAt and endsAt must be valid dates" }, { status: 400 });
  if (startsAt && endsAt && startsAt > endsAt) return NextResponse.json({ error: "endsAt cannot be earlier than startsAt" }, { status: 400 });
