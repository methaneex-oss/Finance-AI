...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const metric = await prisma.financialMetric.findFirst({ where: { id, organizationId, active: true }, include: { inputs: { include: { category: true } } } });
    if (!metric) return NextResponse.json({ error: "Metric not found" }, { status: 404 });

    const entries = await prisma.financialEntry.findMany({
      where: { organizationId, status: "POSTED", ...(from || to ? { entryDate: { ...(from ? { gte: from } : {}), ...(to ? { lt: to } : {}) } } : {}) },
    ...
    return NextResponse.json({ metric: { id: metric.id, name: metric.name, description: metric.description, formula: metric.formula }, inputs, result, currency: organization.baseCurrency, range: { from: from?.toISOString() ?? null, to: to?.toISOString() ?? null } });
  } catch (error) {
